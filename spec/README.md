# Area behavior models

This directory holds the **behavioral specification** this suite targets: a
black-box, forge-neutral model of how [PostgREST](https://postgrest.org)
responds to HTTP requests, distilled from PostgREST's own public behavior and
test suite.

This repository (`postgrest-conformance`) records what PostgREST v16.0
actually does, case by case, so it can be checked automatically. It is a
language- and implementation-agnostic conformance suite: consumers (bier
among them) bring their own PostgreSQL-backed REST API server implementations
to the cases in their own repos, using `HARNESS.md` as the implementer
contract. This `spec/` tree pins down what that compatibility means.
...
> `fixtures/inputs/rpc.sql#L15` carried that stale provenance pin.
> **Both halves of that condition are now closed.** Commit **`6b25f05`**
> re-pinned the fixture: `fixtures/inputs/rpc.sql:15` reads
...
> `INDEX.md`, which the synthesis phase rewrites; counting files
...
> not shrink this pass.** **Six** files under `fixtures/provenance/` carry **43**
...
> `fixtures/provenance/representations.sql` and `fixtures/provenance/openapi.sql` each carry **zero**
...
> `fixtures/provenance/` and no line of `fixtures/02_base.sql` is modified in
> `git status` this pass: the openapi re-sync added **no fixture object**, the
> first re-sync to leave `fixtures/02_base.sql` untouched since content_negotiation's
...
> `fixtures/02_base.sql`), so the re-syncs otherwise leave them alone — but
...
```
(repo root)
├── README.md                  # quick-start overview for implementers
├── CONTRIBUTING.md            # contribution + freeze rules
├── HARNESS.md                 # the implementer contract
├── PIN                        # pinned PostgREST version + commit
...
├── fixtures/
│   ├── 02_base.sql            # the authoritative merged DDL+seed set
│   ├── 03_supplement.sql      # human-owned supplement
│   ├── inputs/                # live generator inputs (rpc.sql, headers.sql)
│   ├── provenance/            # per-area fragments + write-channel deltas (see fixtures/README.md)
│   └── README.md
└── spec/                      # this directory
    ├── README.md               # this file
...
2. **Conformance cases** — 762 YAML files under `cases/`. Each is one
...
> expectations and pass only because the fixture chain's `fixtures/07_analyze.sql`
> runs a database-wide `ANALYZE`.
...
The conformance database is built by the numbered fixture chain
(`fixtures/01_roles.sql`…`fixtures/07_analyze.sql`; see `fixtures/README.md`),
which loads `fixtures/02_base.sql` (the authoritative artifact) plus the
human-owned `fixtures/03_supplement.sql`, followed by area-schema mirroring. A case's
...
> in the loaded conformance database nor a `db_schema_aliases` key; it resolves only because
...
Ownership rules for everything under `fixtures/provenance/` — who may write
which file, and why `<area>.delta.sql` is the only write channel — are in
[`fixtures/README.md`](../fixtures/README.md). There are
...
`-- Folded into ../fixtures/02_base.sql on <date> (…); empty until the next delta.`
...
> (`id public.devil_int DEFAULT 420, name text`) into `fixtures/02_base.sql` section 4
...
> call.** `fixtures/02_base.sql` does not appear in `git status`; `rpc.delta.sql` is
...
> `fixtures/inputs/rpc.sql`, which no workflow agent may edit, so a spec pass can only
...
> modified `fixtures/provenance/observability.sql` — **comment-only**: it
...
See [`INDEX.md`](../INDEX.md) for the full
area ↔ id-band ↔ fixture ↔ label map.

## How this suite is consumed

This `spec/` tree is the output of the spec-research phase: one behavior
model plus a set of conformance cases per PostgREST feature area. Any
PostgreSQL-backed REST API server's test harness can consume it directly —
`HARNESS.md` is the implementer contract: for each YAML file under `cases/`,
execute the HTTP (or CLI) request it describes, collect the response, and
...
never edits the cases to match a server's current behavior. The two scoped
exceptions — re-syncing to a new PostgREST pin via the `bier-spec` /
`bier-spec-audit` workflows, and human-reviewed edits to
`fixtures/03_supplement.sql` — are described in `CONTRIBUTING.md`.
...
> schema that **does not exist** in the loaded conformance database. The
...
> **does** exist (`fixtures/02_base.sql#L247`). **A green case can be green because of a
