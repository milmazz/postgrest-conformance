/**
 * NOTE: this workflow now maintains the standalone postgrest-conformance
 * repository (extracted from milmazz/bier). The name "bier-spec" is kept
 * for continuity with its origin and prior run history — it no longer
 * refers to the bier repo's spec/ subtree.
 *
 ...
 * the primary fixtures/02_base.sql, a machine-check Verify pass validates the tree
 ...
 * COVERAGE.md, INDEX.md (and, in Synthesize, spec/README.md). Does not touch
 ...
 *   - Fixtures: fixtures/02_base.sql is the PRIMARY artifact and is
 ...
 *     the sequential Consolidator folds them in. fixtures/03_supplement.sql and the
 *     live loader inputs fixtures/inputs/rpc.sql / fixtures/inputs/headers.sql are human-owned and untouchable.
...
export const meta = {
  ...
  phases: [
    ...
    { title: "Consolidate", detail: "fold fixtures/provenance/*.delta.sql into the primary fixtures/02_base.sql" },
    ...
    { title: "Synthesize", detail: "spec/README.md, COVERAGE.md, INDEX.md refreshed from disk" },
  ],
};
...
§5 and case.schema.json for the exact deliverable shapes. Work from the
...
touch fixtures/inputs/, fixtures/02_base.sql...07_analyze.sql, tools/, or the
repo config.
...
everything by hand; work the DIFF:
  ...
  - locate this area's existing cases by their \`feature:\` field, e.g.
    grep -rl 'feature: ${area.key}' cases/ — NEVER assume id
...
Deliverables, under spec/:
  ...
  - cases/NNNN_<slug>.yaml  >=1 black-box case per public feature,
       in the case.schema.json case shape (id, feature, request, schema,
       preconditions, expect{status,headers,body_*}, notes, source)
       [NOTE: the preconditions: key was retired in this repo — state
       assumptions go in notes: as "Assumes: ...". See CONTRIBUTING.md]
  - fixtures/provenance/${area.key}.delta.sql  ONLY if your new/changed
...
FIXTURE RULES (fixtures/README.md is authoritative):
  - Seed-data ground truth is the CONSOLIDATED database as built by the
    numbered fixture chain (fixtures/01_roles.sql...fixtures/07_analyze.sql,
    per fixtures/README.md: psql -v ON_ERROR_STOP=1 under PGTZ=UTC, into a
    LC_COLLATE 'C' database) — e.g. test.items has rows 1..15, a superset of
    what any historical fragment seeds. Derive and verify every expected body
    against a freshly loaded scratch database (e.g. conf_scratch; drop it
    afterwards), NEVER against a fragment file.
  - NEVER edit fixtures/02_base.sql, fixtures/03_supplement.sql, or any
    existing fixtures/*.sql (fixtures/inputs/rpc.sql and fixtures/inputs/headers.sql are LIVE loader inputs
    with fragile invariants). New fixture objects go ONLY in
    fixtures/provenance/${area.key}.delta.sql — new objects only, never
    DDL that duplicates what fixtures/02_base.sql already has. The consolidator folds
...
HARD RULES:
  ...
  - If a behavior cannot be expressed with the existing keys in
    case.schema.json, do NOT approximate it with a weaker assertion (e.g.
...
Locate this area's material:
  ...
  - its conformance cases: the cases/*.yaml whose \`feature:\`
    field's leading segment is "${area.key}" (find them with e.g.
    grep -rl 'feature: ${area.key}' cases/). Do NOT rely on id
    ranges — on-disk ids don't all follow a formula.
...
For EVERY entry in the model and EVERY case:
  ...
  3. Validate each case against case.schema.json (note if it is absent),
     and flag any case whose expected body contradicts the CONSOLIDATED seed
     data (a database loaded from the numbered fixture chain —
     fixtures/02_base.sql through 07_analyze.sql, per fixtures/README.md —
     e.g. test.items is rows 1..15), which is the seed ground truth — not the
...
fixtures/02_base.sql is the PRIMARY fixture artifact. NEVER regenerate
...
["check-not-run: <why>"]) and say why in "evidence":
  ...
  4. Stale pins: grep every \`source:\` URL in spec/*.yaml, spec/*.md and
     cases/*.yaml — list every file/case whose URL references
...
write only the suite content — spec/README.md, COVERAGE.md, INDEX.md, and
case.schema.json if absent. REFRESH these to match what is on disk NOW — read the
actual files, never trust stale counts:
  - spec/README.md        overview + the pinned version (${PINNED})
  - COVERAGE.md      a table mapping every PostgREST docs page (${DOCS})
  ...
  - INDEX.md  cross-reference of cases <-> feature areas with
                          per-area counts
  - case.schema.json ONLY if it does not exist yet (draft a faithful
...
// Phase Consolidate: fold the fixture deltas into the primary fixtures/02_base.sql.
log("Folding fixture deltas → fixtures/02_base.sql");
...
if (!consolidateResult) gaps.push({ kind: "consolidate_agent_lost", detail: "consolidator returned no report — delta files may be unfolded; inspect fixtures/provenance/*.delta.sql" });
