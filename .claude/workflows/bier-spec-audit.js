 ...
 * repository (extracted from milmazz/bier). The name "bier-spec-audit" is
 * kept for continuity with its origin and prior run history — it no longer
 ...
 * into the PRIMARY fixtures/02_base.sql (never regenerating it from
 ...
 *   - Fixtures: fixtures/02_base.sql is PRIMARY; fixers write new objects to per-area
 *     *.delta.sql files; fixtures/03_supplement.sql and the live loader inputs
 *     fixtures/inputs/rpc.sql / fixtures/inputs/headers.sql are untouchable (fixtures/README.md).
...
export const meta = {
  ...
  phases: [
    ...
    { title: "Synthesize", detail: "refresh COVERAGE.md + INDEX.md from disk" },
  ],
};
...
Locate this area's material:
  ...
  - its conformance cases: they are the cases/*.yaml whose
...
Rules:
  ...
  - FIXTURE RULES (fixtures/README.md is authoritative):
    ...
    objects go ONLY in fixtures/provenance/${area.key}.delta.sql (new
  ...
  - Every case must still validate against case.schema.json and carry a
...
Work from the repository root; write ONLY under spec/:
  ...
  - INDEX.md refresh the area <-> case-ids <-> fixture
