...
>
> The workflow's name is kept for continuity with its origin (this suite was
> extracted from `milmazz/bier`) and prior run history — it no longer refers
> to that repo's `spec/` subtree. It now maintains this standalone
> `postgrest-conformance` repository.

## 1. What this workflow does

It builds (or re-syncs) the whole conformance suite — the *spec research*
pass — as a single background [dynamic workflow](https://code.claude.com/docs/en/workflows):
...
source, then consolidates and synthesizes the result into a complete suite
tree. The run returns **one report**; no per-turn transcript.

It is deliberately scoped to research only. It writes only the suite
content: `cases/`, `spec/`, `fixtures/provenance/`, `COVERAGE.md`, `INDEX.md`
(and, in Synthesize, `spec/README.md`). It does **not** touch
`fixtures/inputs/`, `fixtures/02_base.sql`…`07_analyze.sql`, `tools/`, or the
repo config — those change only through `CONTRIBUTING.md`'s reviewed routes;
per-area `.delta.sql` files are the write channel. This suite records
PostgREST's behavior; consumers (bier among them) bring their own
implementations to the cases in their own repos — bringing an implementation
up to the cases is a consumer's job, not this workflow's.
...
- Originally this was the bottleneck (no suite tree existed and every later
  phase was blocked on it). Today the tree exists and the workflow's usual job
  is the **sync-mode re-run**: re-verifying and updating the tree against a
  newer pinned PostgREST version.
...
D. Consolidate    1 agent: fold per-area fixture deltas (fixtures/provenance/*.delta.sql)
                  into the PRIMARY fixtures/02_base.sql — NEVER a
                  regeneration from the historical fragments (they lack objects
                  and seed-merge decisions only the merged file has); deltas are
                  emptied after folding. After a fold, fixtures/06_area_schemas.sql
                  must be regenerated (elixir tools/regen_area_schemas.exs) before
                  the full chain re-loads cleanly.
E. Verify         1 agent: MACHINE checks with real command output — fixture load
                  (the numbered chain into a scratch database, or psql), every
                  case validated against case.schema.json, case-id uniqueness,
                  zero `source:` URLs citing a non-pinned version, every
                  case-referenced relation exists in the loaded DB, case count
F. Synthesize     1 agent: spec/README.md, COVERAGE.md, INDEX.md
...
**Sync mode.** When the suite tree already exists (the usual case now),
research agents do NOT regenerate: they diff the previous pin against the
target (`git diff <prev>..<pinned> -- test/spec/ src/`) to find what actually
changed, locate their area's cases by the `feature:` field, re-anchor every
citation to the pinned version, fix changed behavior, add cases for new
features, and drop no-longer-existing behavior as logged gaps. Behaviors the
frozen `case.schema.json` cannot express are never approximated with weaker
assertions — they're recorded as `needed_assertion:` gaps for the human
review gate, and fixture objects needing loader exposure (rpc/headers area
schemas, new `schema:` labels) as `loader_exposure:` gaps.
...
**Fixture layering.** `fixtures/02_base.sql` is the primary artifact;
researchers write new fixture objects only to their area's
`fixtures/provenance/<key>.delta.sql`; `fixtures/03_supplement.sql` (human-owned)
and the live loader inputs `fixtures/inputs/rpc.sql`/`fixtures/inputs/headers.sql`
are untouchable. Seed-data ground truth for expected bodies is the CONSOLIDATED
database as built by the numbered fixture chain (`fixtures/01_roles.sql`
through `fixtures/07_analyze.sql`, per `fixtures/README.md`), never a
historical fragment. Full contract: `fixtures/README.md`.
...
- **Writes** (only the suite content):
  - `spec/<key>.yaml` — the area's behavior model.
  - `cases/NNNN_<slug>.yaml` — ≥1 black-box case per public
    feature in the area, in the `case.schema.json` shape (request →
    expect, with `source`).
  - `fixtures/provenance/<key>.delta.sql` — only if new/changed cases need
    fixture objects that don't exist yet (new objects only; folded into the
    primary `fixtures/02_base.sql` by phase D).
...
- **Verifies** each citation actually supports the claim (re-fetches the source),
  ...
  and lints every case against `case.schema.json`.
...
Folds every `fixtures/provenance/*.delta.sql` into the **primary**
`fixtures/02_base.sql` (dependency-correct position, dedup against
what's already there, rename real collisions), then empties each folded delta
and verifies the load (numbered chain into a scratch database, falling back to
psql / static check). After folding, `fixtures/06_area_schemas.sql` must be
regenerated (`elixir tools/regen_area_schemas.exs`) before the full chain
re-loads cleanly. It must NEVER regenerate `fixtures/02_base.sql` from the
historical fragments and never touch `fixtures/03_supplement.sql` or the live
loader inputs `fixtures/inputs/rpc.sql`/`fixtures/inputs/headers.sql`. Returns
the folded-delta and conflict lists.

### E · Synthesis agent (one)
Emits `spec/README.md` (version pin + overview), `COVERAGE.md` (every
PostgREST docs page → covering case IDs), a draft `case.schema.json`
(JSON-Schema the cases validate against — this repo's maintainers own it
thereafter), and `INDEX.md`. Returns coverage totals.
...
├── openapi.yaml  config.yaml  observability.yaml  domain_representations.yaml
COVERAGE.md   case.schema.json   INDEX.md
cases/NNNN_*.yaml       # black-box request→response records
fixtures/
├── 02_base.sql            # consolidated, loads on PG 15/16/17
├── 03_supplement.sql      # human-owned supplement (never workflow-written)
├── README.md              # fixture layering + ownership contract
├── inputs/                # live generator inputs: rpc.sql, headers.sql
└── provenance/<key>.sql   # historical fragments (provenance only)
    provenance/<key>.delta.sql # transient write channel, emptied after folding
```

## 7. Exit criteria (the suite tree is done)

- `fixtures/02_base.sql` loads cleanly into Postgres 15/16/17 via the numbered
  chain.
- ≥1 conformance case per public feature in PostgREST's docs ToC.
- Every case YAML validates against `case.schema.json`.
- `COVERAGE.md` maps every PostgREST docs page → its case IDs.
...
3. **Launch.** `bier-spec.js` lives in `.claude/workflows/`, which is *not* a
   ...
   - **(Re)generate from this brief** instead of using the committed script —
     ...
     fan-out for PostgREST v16.0, one subagent per feature area writing the
     suite tree, a fresh adversarial reviewer re-auditing each area's cited
     sources, then consolidate fixtures and synthesize COVERAGE.md. Write only
     the suite content.
     ```
...
5. **Read the report**, triage its `gaps_for_human_review` list, then hand
   the result to whichever consumer repo (e.g. bier) is bringing its
   implementation up to the cases.
...
- **Structured outputs** (2026-07 revision): agents report via the runtime's
  ...
  kept small (paths/ids/gaps, not full entry dumps); the files on disk are
